...
@router.get("/test-version-151-issues")
async def test_version_151_issues():
    """레드마인에서 151번 버전 위키 페이지 첨부파일 정보 조회 테스트"""
    try:
        # 151번 버전 자체 정보 조회
        version_url = f"{REDMINE_URL}/versions/151.json"
        headers = {"X-Redmine-API-Key": API_KEY}
        ...
        if version_response.status_code != 200:
            return {
                "success": False,
                "message": f"151번 버전 조회 실패 - 상태 코드: {version_response.status_code}",
                "data": {}
            }
        ...
        # 위키 페이지 정보 조회
        project_id = version.get('project', {}).get('id')
        version_name = version.get('name')
        
        if project_id and version_name:
            # 위키 페이지 이름을 URL 형식으로 변환 (공백을 _로, 한글 인코딩)
            wiki_page_name = version_name.strip().replace(' ', '_')
            # 첨부파일을 포함한 위키 페이지 조회 시도
            wiki_url = f"{REDMINE_URL}/projects/samsung/wiki/{wiki_page_name}.json?include=attachments"
            print(f"위키 페이지 조회 URL: {wiki_url}")
            
            wiki_response = requests.get(wiki_url, headers=headers, timeout=10)
            print(f"위키 페이지 조회 응답 상태: {wiki_response.status_code}")
            
            if wiki_response.status_code == 200:
                wiki_data = wiki_response.json()
                wiki_page = wiki_data.get("wiki_page", {})
                
                print(f"위키 페이지 제목: {wiki_page.get('title')}")
                print(f"위키 페이지 내용 길이: {len(wiki_page.get('text', ''))}")
                print(f"위키 페이지 전체 데이터: {wiki_page}")
                
                # 첨부파일 정보만 출력
                attachments = wiki_page.get('attachments', [])
                if attachments:
                    print(f"\n=== 첨부파일 정보 ({len(attachments)}개) ===")
                    for attachment in attachments:
                        print(f"ID: {attachment.get('id')}")
                        print(f"파일명: {attachment.get('filename')}")
                        print(f"타입: {attachment.get('content_type')}")
                        print(f"크기: {attachment.get('filesize')}")
                        print(f"다운로드 URL: {REDMINE_URL}/attachments/download/{attachment.get('id')}/{attachment.get('filename')}")
                        print(f"직접 URL: {REDMINE_URL}/attachments/{attachment.get('id')}")
                        print("---")
                else:
                    print("첨부파일: 없음")
            else:
                print(f"위키 페이지 조회 실패: {wiki_response.status_code}")
                print(f"응답 내용: {wiki_response.text}")
        
        return {
            "success": True,
            "message": "151번 버전 첨부파일 정보 조회 완료",
            "data": {
                "attachments": wiki_page.get('attachments', []) if 'wiki_page' in locals() else []
            }
        }
        ...
        return {
            "success": False,
            "message": f"151번 버전 조회 중 오류 발생: {str(e)}",
            "data": {
                "error": str(e)
            }
        }
...
def get_wiki_content(wiki_url: str) -> Dict:
    ...
    try:
        # 레드마인 API를 사용해서 위키 내용 가져오기
        # URL에서 project와 page 정보 추출
        # 예: https://redmine.example.com/projects/project-name/wiki/Page-Name
        import re
